...
export default function HeadOfDeptApplicationsPage() {
  ...
  const getFilteredApplications = () => {
    ...
    return applications.filter(app => {
        ...
        if (activeTab === 'PENDING') {
            return app.status === 'PENDING_DEPT_HEAD';
        }
        
        if (activeTab === 'APPROVED') {
             // Show applications that have been accepted by HoD and moved forward
             return [
                 ...
                 'PENDING_COMMITTEE', 
                 'APPROVED',
                 'REJECTED' // Include final rejected if it passed HoD? 
             ].includes(app.status || '');
        }

        if (activeTab === 'REJECTED') {
             // Show applications rejected strictly by HoD
             return app.status === 'REJECTED_BY_DEPT_HEAD';
        }
  ...
  const handleApprove = async (id: string) => {
    if (window.confirm('Are you sure you want to approve this application?')) {
        const success = await updateStatus(id, 'APPROVED', 'Approved by Head of Department'); 
        if (success) {
  ...
  const handleBulkApprove = async (ids: string[]) => {
    if (window.confirm(`Approve ${ids.length} applications?`)) {
        let successCount = 0;
        for (const id of ids) {
            const success = await updateStatus(id, 'APPROVED', 'Bulk Approved by Head of Department');
        ...
        <div className="bg-white border-b border-gray-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
                    <button onClick={() => router.back()} className="hover:text-gray-900">Department</button>
                    <span>/</span>
                ...
                {/* Optional: Kept the department info but styled subtly to match the clean UI */}
                {user?.department?.name && (
                    <p className="text-gray-500 mt-2 text-sm">Department: {user.department.name}</p>
                )}
