...
export default function ViceDeanApplicationDetailPage() {
  ...
  useEffect(() => {
    if (!userLoading) {
      if (!user) {
        router.push('/');
      } else if (user.role !== 'VICE_DEAN') { // Changed role check
        router.push('/');
      }
    }
  ...
  useEffect(() => {
    if (id && user && user.role === 'VICE_DEAN') {
      fetchApplication();
    }
  ...
  // Changed status check to Vice Dean stage
  const isPending = application.status === 'PENDING_VICE_DEAN';
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {/* Updated label */}
              <p className="text-xs text-gray-500">รองคณบดี · ตรวจสอบใบสมัคร</p>
            </div>
          </div>
          <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Action Panel — only shown when status is PENDING_VICE_DEAN */}
        {isPending && (
          <div className="pt-8 border-t border-gray-200">
            <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
